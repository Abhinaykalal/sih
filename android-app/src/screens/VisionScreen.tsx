    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer} showsVerticalScrollIndicator={false}>
      <View style={styles.headerRow}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.screenSub}>Analyze the actual uploaded leaf image. No demo or fallback diagnosis.</Text>
        </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.cropRow}>
        {CROPS.map((crop) => <TouchableOpacity key={crop} style={[styles.cropChip, selectedCrop === crop && styles.cropChipActive]} onPress={() => setSelectedCrop(crop)}><Text style={[styles.cropChipText, selectedCrop === crop && styles.cropChipTextActive]}>{crop}</Text></TouchableOpacity>)}
      </ScrollView>
      ...
      {result && <View style={[styles.resultCard, !predictionAvailable && styles.resultCardWarning]}>
        ...
        {predictionAvailable ? <>
          <View style={styles.metricsRow}><View style={styles.metricBox}><Text style={styles.metricBoxLabel}>Confidence</Text><Text style={styles.metricBoxVal}>{(result.confidence_pct ?? 0).toFixed(1)}%</Text></View><View style={styles.metricBox}><Text style={styles.metricBoxLabel}>Crop context</Text><Text style={styles.metricBoxVal}>{selectedCrop}</Text></View><View style={styles.metricBox}><Text style={styles.metricBoxLabel}>Model</Text><Text style={styles.metricBoxVal}>Experimental</Text></View></View>
          <View style={styles.divider} /><View style={styles.treatmentSection}><Text style={styles.treatmentTitle}>Recommended next step</Text><Text style={styles.treatmentBody}>{result.action || 'Confirm the result with a qualified agronomist.'}</Text></View>
          {!!result.warning && <Text style={styles.warningText}>{result.warning}</Text>}
        </> : <View style={styles.treatmentSection}><Text style={styles.treatmentBody}>{result.action || 'No diagnosis was made.'}</Text><Text style={styles.warningText}>No disease name or confidence is shown because the image was not reliably analyzed.</Text></View>}
      </View>}
...
function formatStatus(status?: string) { return (status || 'NO_RELIABLE_RESULT').replace(/_/g, ' '); }

const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#F8FAF8' }, contentContainer: { padding: 16, paddingBottom: 110 },
  headerRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }, screenTitle: { fontSize: 18, fontWeight: '800', color: theme.colors.textPrimary }, screenSub: { fontSize: 12, color: theme.colors.textSecondary, marginTop: 2, paddingRight: 8 }, cropRow: { marginBottom: 14 },
  cropChip: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#E2E8F0', marginRight: 8 }, cropChipActive: { backgroundColor: theme.colors.primary, borderColor: theme.colors.primary }, cropChipText: { fontSize: 11, fontWeight: '700', color: '#64748B' }, cropChipTextActive: { color: '#FFFFFF' },
  scannerCard: { backgroundColor: '#FFFFFF', borderRadius: 14, padding: 16, marginBottom: 14, borderWidth: 1, borderColor: '#E2E8F0' }, preview: { width: '100%', height: 230, borderRadius: 12, backgroundColor: '#E2E8F0' }, placeholder: { height: 230, borderRadius: 12, backgroundColor: '#0F172A', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 20 }, scannerPrompt: { color: '#F8FAFC', fontSize: 13, fontWeight: '700', textAlign: 'center', marginTop: 8 }, scannerTip: { color: '#94A3B8', fontSize: 11, marginTop: 4, textAlign: 'center' },
  actionRow: { flexDirection: 'row', gap: 8, marginTop: 12 }, secondaryBtn: { flex: 1, minHeight: 48, borderRadius: 10, backgroundColor: '#F1F5F9', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 6 }, secondaryBtnText: { color: '#334155', fontSize: 12, fontWeight: '700', textAlign: 'center' }, resetBtn: { minWidth: 58, borderRadius: 10, backgroundColor: '#FEE2E2', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 8 }, resetBtnText: { color: '#B91C1C', fontSize: 12, fontWeight: '800' },
  primaryScanBtn: { marginTop: 12, backgroundColor: theme.colors.primary, borderRadius: 10, paddingVertical: 13, alignItems: 'center' }, primaryScanBtnDisabled: { opacity: 0.5 }, primaryScanBtnText: { color: '#FFFFFF', fontSize: 14, fontWeight: '700' },
