...
import {
  Bell, ChevronRight, CloudSun, Droplets, Home, Leaf, Lightbulb, MapPin,
  Menu, MessageCircle, MoreHorizontal, Plus, ScanLine, Settings, Sprout,
  ThermometerSun, Tractor, TrendingUp, Waves, Zap,
} from "lucide-react";
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!mounted) return;
        if (sensor.status === "fulfilled") { setTelemetry(sensor.value); setLive(true); }
  ...
  const togglePump = async (next: boolean) => {
    ...
    setLoadingPump(true);
    try {
      await sendPumpCommand(next ? "ON" : "OFF");
      setPumpOn(next);
      showToast(next ? "Pump command sent." : "Pump stop command sent.");
    } catch {
      showToast("Pump command failed. Check the backend connection.");
        ...
        <div className="page-wrap">
          {activeTab === "home" && <>
            ...
            <section className="hero-grid">
              <article className="weather-card"><div className="weather-image"><div className="sun"><CloudSun size={50} strokeWidth={1.5} /></div><div className="field-horizon" /><div className="tractor"><Tractor size={42} /></div></div><div className="weather-info"><div><span className="tiny-label">FIELD CONDITIONS</span><strong>{Math.round(num(telemetry.temperature, 32))}°</strong><span>{live ? "Live sensor environment" : "Weather preview"}</span></div><div className="weather-stats"><span><Droplets size={14} /> {Math.round(humidity)}% humidity</span><span><Waves size={14} /> {live ? "Sensor online" : "12 km/h wind"}</span></div></div></article>
              <article className="insight-card"><div className="card-topline"><span className="ai-badge"><Lightbulb size={15} /> Saathi AI</span><span>{live ? "Based on live telemetry" : "Preview insight"}</span></div><h2>{moisture < 35 ? "Water your wheat tonight." : "Your wheat is in a good moisture range."}</h2><p>{moisture < 35 ? `Soil moisture is ${Math.round(moisture)}%. A controlled irrigation cycle can move Plot F5 back toward its target range.` : `Soil moisture is ${Math.round(moisture)}%. Keep monitoring the field and follow the next recommended agronomic action.`}</p><div className="insight-bottom"><div className="mini-metric"><span>Soil moisture</span><strong>{Math.round(moisture)}%</strong></div><button onClick={() => setActiveTab("pump")}>Review irrigation <ChevronRight size={16} /></button></div></article>
            </section>
            ...
            <section className="bottom-cta"><div className="cta-icon"><MessageCircle size={22} /></div><div><strong>Need a second opinion?</strong><span>Ask Saathi about your crop, soil or next decision.</span></div><button onClick={() => showToast("AI assistant opened.")}>Ask Saathi <ChevronRight size={17} /></button></section>
          </>}
          {activeTab === "fields" && <FieldsView onToast={showToast} />}
          {activeTab === "leaf" && <LeafView onToast={showToast} />}
          {activeTab === "pump" && <PumpView pumpOn={pumpOn} setPumpOn={setPumpOn} onToggle={togglePump} loading={loadingPump} />}
