...
class IrrigationAssessmentResult(BaseModel):
    ...
    # Provenance traceability fields (per audit items 169-173)
    threshold_source: str = "ICAR/FAO published guidelines — not empirically validated against this farm's field outcomes"
    ...
    domain_disclaimer: str = (
        ...
        "ICAR/FAO moisture threshold guidelines. It has NOT been validated against real farm "
        "field outcomes or agronomist-reviewed performance data. Always consult a qualified "
        "agronomist before acting on this advisory for commercial agricultural decisions."
    )
    ...
    def assess_irrigation_needs(
        ...
        if c_lower in ["rice", "paddy"]:
            if "harvest" in st_lower or "ripen" in st_lower:
                ...
                band_source = "ICAR-IIRR Water Management in Rice, 2018 Ed. §4.2 — Pre-harvest drying phase"
            else:
                min_thresh, max_thresh = 40.0, 60.0
                band_desc = "40.0%–60.0% (Vegetative shallow water / AWD)"
                band_source = "ICAR-IIRR Package of Practices for Rice, FAO Alternate Wetting & Drying (AWD) Guidelines"
        elif c_lower in ["wheat"]:
            min_thresh, max_thresh = 35.0, 50.0
            band_desc = "35.0%–50.0% (Crown root & tillering)"
            band_source = "ICAR-IARI Wheat Agronomy Bulletin, 2020 Ed. §3.1 — Critical moisture periods"
        else:
            min_thresh, max_thresh = 30.0, 50.0
            band_desc = "30.0%–50.0% (Standard root zone)"
            band_source = "FAO Crop Water Requirements, Irrigation & Drainage Paper 56 (generalized default)"
